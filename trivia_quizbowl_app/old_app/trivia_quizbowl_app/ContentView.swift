...
//
// Maybe make a timer on screen to see when it will autosubmit
// Show us the answer to the previous question when we start a new one
// Talk about category difficulty
//manual override if the answer they gave is the same but they counted it wrong
...
//    }
    var body: some View {
        NavigationView{
            ZStack(){
                RadialGradient(gradient: Gradient(colors: [Color(#colorLiteral(red: 0.4745098054, green: 0.8392156959, blue: 0.9764705896, alpha: 1)), tryAgainOrCorrect]), center: .center, startRadius: 5/*@END_MENU_TOKEN@*/, endRadius: /*@START_MENU_TOKEN@*/500/*@END_MENU_TOKEN@*/).edgesIgnoringSafeArea(/*@START_MENU_TOKEN@*/.all)
                VStack() {
                  ...
                  //      HStack(){
                       ...
                       //   Button(action: endRound) {
                         //       Text("New Question").padding().background(colorScheme[1]).foregroundColor(Color(#colorLiteral(red: 0, green: 0, blue: 0, alpha: 1))) //arnav's line = adding an endround button
                           // }
                    //    }
                        } // gets a new question
