    ...
    if data["difficulty"] == 11:
        for item,value in topics_to_get.items():
            for x in range(value):
                ...
                final_questions.append({"question":parse_question(question[1]),"questionId":question[0],"answer":parse_question(question[2]),"topic":question[3],"difficulty":question[4]})
    else:
        for item,value in topics_to_get.items():
            for x in range(value):
...
def leave_room(data):
    room = dbs_worker.leave_room(data['token'],data['roomId'])
    if len(room['users']) == 0:
        dbs_worker.deactivate_room(data['roomId'])
    else:
        emit('roomLeft',room,to=room)

@socketio.on('getRoom')
def get_room(data):
    room = dbs_worker.get_room_by_id(data['roomId'])
    emit('room',room)

@socketio.on('startRoomGame')
def start_room_game(data):
    room = dbs_worker.start_room_game(data['roomId'],data['settings'])
    emit('roomGameStarted',room,to=room["roomId"])
