...
def get_full_question_with_specific_difficulty_and_topic(difficulty,topic):
    db_connection = connect_to_datbase("localhost","smalani",current_db_g)
    command = "SELECT * FROM original_questions TABLESAMPLE SYSTEM(1) where difficulty=(%s) and topic=(%s) and status=(%s) limit 1;"
    data = (difficulty,topic,1)
    question = None
    while question == None:
        get_question_data = execute_database_command_with_data(db_connection,command,data)
